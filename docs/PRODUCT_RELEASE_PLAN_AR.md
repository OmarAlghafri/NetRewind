...
>
> **حالة التنفيذ:** ما أُنجز فعليًا مسجَّل في `CHANGELOG.md`، وما اختُبر منه
> موثَّق بسجلات حقيقية في `docs/evidence/` (وفهرسها في `docs/testing.md`)، لا
> في هذا الملف — هذا المستند يبقى الخطة كما كُتبت، والتقدم الفعلي يُقاس بدليل
> قابل للتحقق في تلك الملفات، لا بتعديل النص هنا.
