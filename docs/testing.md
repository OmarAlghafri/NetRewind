...
| Desktop shell | `make desktop-resources && cargo test --lib --manifest-path desktop/src-tauri/Cargo.toml` (the shell refuses to compile without the staged resource directory) | any | evidence-bundle verification (checksums, tampering, missing members, newer formats, signature required/valid/wrong key), launch-option parsing |
